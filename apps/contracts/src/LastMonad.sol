...
contract LastMonad is Ownable, ReentrancyGuard {
    ...
    uint256 public constant BASE_STAKE = 2 ether; // 2 MON - Base stake for pool creation
    uint256 public constant POOL_MULTIPLIER = 100; // 1x multiplier (100/100) - fair 1:1 ratio
    uint256 public constant MAX_STAKE_ALLOWED = 200 ether; // Cap at 200 MON maximum stake
    uint256 public constant PENALTY_PERCENTAGE = 30;
    uint256 public constant CREATOR_REWARD_PERCENTAGE = 12; // 12% creator reward - attractive incentive for pool creation
    ...
    function stakeForPoolCreation() external payable {
        require(msg.value >= BASE_STAKE, "Minimum stake is 2 MON");
        require(msg.value <= MAX_STAKE_ALLOWED, "Maximum stake is 200 MON");
    ...
    function createPool(uint256 _entryFee, uint256 _maxPlayers) external {
        require(poolCreators[msg.sender].hasActiveStake, "Must stake MON to create pools");
